...
from .mixins import ClientLoggedMixin, ObjectDataMixin, ObjectsDeleteMixin, ReqGetMixin

USER = get_user_model()


def get_next_page(page):
    return f"?limit={DEFAULT_PAGE_NUMBER_LIST}&limite={DEFAULT_PAGE_NUMBER_LIST}&offeset={DEFAULT_PAGE_NUMBER_LIST}&offset={DEFAULT_PAGE_NUMBER_LIST}&ofset={DEFAULT_PAGE_NUMBER_LIST}&page={page}"
...
class product_detail_test(
    ...
    def setUp(self):
        ...
        self.product = baker.make(
            Product, name=self.product_name, slug=self.product_slug
        )
...
class product_delete_test(ProductTestBase):
    def setUp(self):
        self.user = baker.make(USER)
...
class category_list_test(CategoryTestBase):
    def setUp(self):
        self.url = reverse("product:category-list")

    def test_if_max_obj_by_page_is_correct(self):
        category_number = len(
            baker.make(self.model, _quantity=DEFAULT_PAGE_NUMBER_LIST)
        )
